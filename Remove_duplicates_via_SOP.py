...
def get_dicom_identifier(dicom_file):
    """Get the SOPInstanceUID from a DICOM file to check for duplicates."""
    ds = pydicom.dcmread(dicom_file)
    sop_instance_uid = ds.SOPInstanceUID
    return sop_instance_uid

def process_file(file_path):
    """Process a single DICOM file and return its SOPInstanceUID and path."""
    try:
        sop_uid = get_dicom_identifier(file_path)
        return sop_uid, file_path
    ...
    with tqdm(total=total_files, desc="Processing DICOM files") as pbar:
        with ProcessPoolExecutor() as executor:
            ...
            for future in as_completed(future_to_file):
                ...
                if result:
                    sop_uid, file_path = result
                    dicom_identifiers[sop_uid].append(file_path)
                pbar.update(1)
...
def map_duplicates(duplicates):
    for sop_uid, file_paths in duplicates.items():
        print(f"Found duplicates for SOPInstanceUID {sop_uid}:")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Find and remove duplicate DICOM files based on SOPInstanceUID.")
